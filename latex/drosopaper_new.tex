...
Paul Graham$^{3}$
\\
\bf{1} Department of Informatics, University of Sussex, Falmer, Brighton, United Kingdom,
\\
...
\\
\bf{3} School of Life Sciences, University of Sussex, Falmer, Brighton, United Kingdom
\\
$\ast$ andrewop@sussex.ac.uk
\end{flushleft}
...
Performance on poorly discriminated patterns can be improved, however, by simply adding more RFs of the same form. Figure~\ref{fig:simdiffpatts}D and E show the increase in performance with number of RFs for two such pattern pairs: triangles and triangularly shaped horizontal bars aligned about the centre of mass (from pattern set~\emph{(9)} in Figure~\ref{fig:pattern}; see Materials and methods for details). This demonstrates that the patterns could be discriminated by flies simply with the addition of more RFs centred on other portions of the visual field.
...
Flies show an innate attraction to long bar-like objects, on which they might perch \cite{Maimon2008} and in Figure~\ref{fig:naturalstim} we show one example of how behaviourally relevant information is maintained in the output of R4d cells even for complex scenes.
...
\begin{figure}[htp]
	\caption{
		{\bf Mean output of R4d RFs as they move azimuthally across two natural panoramic images.}
...
\begin{figure}[htp]
	\caption{
		...
		Note that this is the left-side version; the right-side version is its mirror.
...
\end{array}
\end{equation}
where $I_{i,j}(\theta)$ and $K_{i,j}$ are the ($i,j$)th pixels of the image and kernel, respectively.
...
% We have left this section in, even though it is empty, as specified in the TEX template file.

\begin{comment}
% funding information
AD, PG and AP are funded by Engineering and Physical Sciences Research Council (https://www.epsrc.ac.uk) under grant code EP/P006094/1.
AW is funded by the Fyssen Foundation (http://www.fondationfyssen.fr/en).
AD and PG have also received funding from the Biotechnology and Biological Sciences Research Council (http://www.bbsrc.ac.uk) under grant codes BB/F015925/1 and BB/H013644.
AP has also received funding from the European Union’s Seventh Framework Programme for research, technological development and demonstration (https://ec.europa.eu/research/fp7) under grant agreement no. 308943.
