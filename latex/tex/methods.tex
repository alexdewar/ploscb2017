...
\label{sec:drosopattern:methods:seelig}
...
The goal of Seelig and Jayaraman's \citeyear{Seelig2013} work was to examine responses of lateral triangle microglomeruli (the ring neurons) to visual stimuli.
...
\label{sec:drosopattern:methods:preprocessing}
...
As data is given for multiple flies, we averaged the RFs for the different glomeruli across flies ($2\le N(\mathrm{R2}) \le 6, 4\le N(\mathrm{R4})\le 7$). This process is summarised in Figure~\ref{fig:drosopattern:avkernels}. Each point on the image was assigned a value ranging from --1 for maximum inhibition to 1 for maximum excitation, based on the values given by the colour scale bars in \citeA{Seelig2013}.
...
g(i,j) = \left\{ \begin{array}{rl}
          ...
          \right.
$$
where $g(i,j)$ is the ($i,j$)th pixel of the kernel, $R_{i,j}$ is the ($i,j$)th value of the processed receptive field image and $T$ is the threshold value, here $0.25$ (Figure~\ref{fig:drosopattern:avkernels}A).
...
$$
where $\hat{g}(i,j)$ is the recentred kernel (Figure~\ref{fig:drosopattern:avkernels}C).
...
where $I_{i,j}(\theta)$ and $K_{i,j}$ are the ($i,j$)th pixels of the image and kernel, respectively. This process is illustrated in Figure~\ref{fig:drosopattern:avkernels}A.

\input{drosopattern/drosopattern_fig_averagekern}

\subsection{Replication of behavioural experiments}
\label{sec:drosopattern:methods:replication}
The equation for describing the bar fixation mechanism shown in Figure~\ref{fig:drosopattern:recap}C is as follows:
$$
...
For the pattern recognition tasks (see Figure~\ref{fig:drosopattern:pattern}), the difference in activation is calculated as follows:
$$
...
\label{sec:drosopattern:methods:neuralnetworks}
...
\label{sec:drosopattern:methods:stimuli}
