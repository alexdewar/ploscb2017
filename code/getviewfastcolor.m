function im=getviewfastcolor(x,y,z,th,X,Y,Z,col,im_size,el_max,orig_im_size,pitch)
% function im=getviewfast(x,y,z,th,X,Y,Z,av_area,el_max,orig_im_size,pitch)
%     minshapesize = 1;

    if nargin < 11 || isempty(orig_im_size)
        orig_im_size = [170 900];
    end
    if nargin < 10 || isempty(el_max)
        el_max = 5*pi/12;
    else
    ...
    if nargin < 8
        ...
        cs2 = size(col,2);
        if size(col,1)==1
            col = repmat(col,nshapes,1);
        end
    end

    im = ones([orig_im_size,size(col,2)]);
    vdist = inf(orig_im_size);

    [az1,el,r] = cart2sph(X-x,Y-y,Z-z);
...
%         keyboard
        ...
        %% add to master image
        cr = mean(r(cind));
        veli = ello+(1:size(cim,1));
        vazi = azlo+(1:size(cim,2));
        vsel = cr <= vdist(veli,vazi);
        vdist(veli,vazi) = min(cr,vsel);
        
        [celi,cazi] = find(cimfill & vsel);
        imind = sub2ind(size(im),repmat(ello+celi,1,cs2),repmat(azlo+cazi,1,cs2),repmat(1:cs2,length(celi),1));
        im(imind) = repmat(col(i,:),length(celi),1);
%         eli = ello+(1:cimht);
%         azi = azlo+(1:cimwd);
