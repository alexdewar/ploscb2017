function im=getviewfastcolor(x,y,z,th,X,Y,Z,cols,av_area,el_max,orig_im_size)
...
if size(cols,1)==1
    cols = repmat(cols,[nshapes,1]);
elseif size(cols,1)~=nshapes
    error('num of colors must match number of shapes')
end

cols = 1-shiftdim(cols,-1);

im = ones([orig_im_size,3]);

[az1,el] = cart2sph(X-x,Y-y,Z-z);
az = mod((size(im,2)-1)*(th-az1+pi)/(2*pi),size(im,2));
...
%     disp(2-loel);
    ...
    cim_color = 1-bsxfun(@times,cim,cols(1,i,:));
