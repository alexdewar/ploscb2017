...
%         xlabel('r.m.s. difference between 0^\circ and 90^\circ')
    ...
    if dosave
        ...
        outfn = sprintf('%s/corrections_panoconv_%s_%s.txt',mfiledir,datestr(now,'yyyymmdd'),mfilehash);
    ...
    function figcorr(Y,whrow,whindex,ylab)
        ...
        if (strcmp(whindex, 'DCP'))
            outliers = cr2X > 0.7;
            
            [rhor2_ol,pr2_ol] = corr(cr2X(~outliers),cY(~outliers),'type','Spearman');
%             [rhorol,prol] = corr(crolX,cY,'type','Spearman');

            outtxt = sprintf('%sCORRELATION WITHOUT OUTLIERS (%s):\n==== %s ====\nR2: N = %d; rho = %f; p = %f\n\n', ... %\nROL: N = %d; rho = %f; p = %f\n\n', ...
                     outtxt,joinstr(', ', num2str(find(outliers))),whindex,sum(~outliers),rhor2_ol,pr2_ol);
        else
            outliers = false(size(cr2X));
        end
        ...
        alsubplot(whrow,1)
        set(gca,'FontSize',8,'FontName','Arial')
        plot(cr2X(~vsel),cY(~vsel),'kx',cr2X(vsel),cY(vsel),'rx',cr2X(outliers),cY(outliers),'bo','MarkerSize',scattermarkersize)
        bestfit(cr2X,cY)
        if whrow==4
        ...
        alsubplot(whrow,2)
        set(gca,'FontSize',8,'FontName','Arial')
        plot(crolX(~vsel),cY(~vsel),'kx',crolX(vsel),cY(vsel),'rx',crolX(outliers),cY(outliers),'bo','MarkerSize',scattermarkersize)
        if whrow==4
