function genellblobs_offset90(starti,nvar,nwave,maxfreq,maxamp,job)
    if ~nargin
        debug = false;
    ...
    suffix = '_azimuths_offset90_noel';
    a2b = 0.5;
...
%     load(['ml_inputs' suffix],'a1','rots1','chts1');
%     ind = wh*ngen+(1:ngen);

    orient = linspace(0,90,100);
    el = 0;
    
    majoraxis = linspace(0,60,101);
    majoraxis = majoraxis(2:end);
    
    [orient,majoraxis] = meshgrid(orient,majoraxis);
    orient = orient(:);
    majoraxis = majoraxis(:);
...
%     zerorad = NaN(nvar,1);
    ...
    x_kern = NaN([nvar,length(kerns)]);
    if ~debug
        startprogbar(50,nvar);
    end
    for i = 1:nvar
        while true
...
% 
if ~debug
    fname = sprintf('%s/ellblob%s%05d_%05d.mat',mfiledir,suffix,job,starti);
    fprintf('Saving to %s...\n',fname);
    save(fname,'x_im','x_kern','majoraxis','minoraxis','im_size','scale','amp','phi','freq','maxfreq','maxamp','nvar','nwave','job','starti','area','orient');
end