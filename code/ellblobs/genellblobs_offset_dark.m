function genellblobs_offset_dark(starti,nvar,nwave,maxfreq,maxamp,job)
    if ~nargin
        debug = false;
...
% % % % % %     function genwaveblobs2(ind)
    ...
    load('ml_inputs_azimuths_offset_dark','a1','rots1','chts1');
%     ind = wh*ngen+(1:ngen);
...
%             figure(1);clf
        ...
        act = rkernmult(rkerns,im);
%         [act,kerns] = getneuronactivations(~im,kerns);
        x_kern(i,:) = act';
...
% 
if ~debug
    fname = sprintf('%s/ellblob_offset_dark%05d_%05d.mat',mfiledir,job,starti);
    fprintf('Saving to %s...\n',fname);
